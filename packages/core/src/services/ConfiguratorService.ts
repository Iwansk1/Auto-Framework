...
export class ConfiguratorService {
    ...
    selectWheels(
        ...
        const wheels =
            this.options.wheels.find((w) => w.id === wheelId) ?? null;
    ...
    togglePackage(
        ...
        const alreadySelected = config.selectedPackages.some(
            (p) => p.id === packageId,
        );
    ...
    private recalculate(config: VehicleConfiguration): VehicleConfiguration {
        ...
        const packagePrice = config.selectedPackages.reduce(
            (sum, pkg) => sum + pkg.priceModifier,
            0,
        );
        ...
        return {
            ...config,
            totalPrice:
                config.vehicle.price + colorPrice + wheelPrice + packagePrice,
        };
    }
}
